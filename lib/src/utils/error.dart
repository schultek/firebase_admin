...
class FirebaseAppError extends _PrefixedFirebaseError {
  ...
  FirebaseAppError.duplicateApp(String message) : this('duplicate-app', message);
  FirebaseAppError.internalError(String message) : this('internal-error', message);
  FirebaseAppError.invalidAppName(String message) : this('invalid-app-name', message);
  FirebaseAppError.invalidAppOptions(String message) : this('invalid-app-options', message);
  FirebaseAppError.invalidCredential(String message) : this('invalid-credential', message);
  FirebaseAppError.networkError(String message) : this('network-error', message);
  FirebaseAppError.networkTimeout(String message) : this('network-timeout', message);
  FirebaseAppError.noApp(String message) : this('no-app', message);
  FirebaseAppError.unableToParseResponse(String message) : this('unable-to-parse-response', message);
}
...
class FirebaseAuthError extends _PrefixedFirebaseError {
  ...
  FirebaseAuthError.claimsTooLarge([String? message])
      : this('claims-too-large', message ?? 'Developer claims maximum payload size exceeded.');
  ...
  FirebaseAuthError.emailAlreadyExists([String? message])
      : this('email-already-exists', message ?? 'The email address is already in use by another account.');

  /// The provided Firebase ID token is expired.
  FirebaseAuthError.idTokenExpired([String? message])
      : this('id-token-expired', message ?? 'The provided Firebase ID token is expired.');

  /// The Firebase ID token has been revoked.
  FirebaseAuthError.idTokenRevoked([String? message])
      : this('id-token-revoked', message ?? 'The Firebase ID token has been revoked.');
  ...
  FirebaseAuthError.invalidClaims([String? message])
      : this('invalid-claims', message ?? 'The provided custom claim attributes are invalid.');

  /// The continue URL must be a valid URL string.
  FirebaseAuthError.invalidContinueUri([String? message])
      : this('invalid-continue-uri', message ?? 'The continue URL must be a valid URL string.');

  /// The creation time must be a valid UTC date string.
  FirebaseAuthError.invalidCreationTime([String? message])
      : this('invalid-creation-time', message ?? 'The creation time must be a valid UTC date string.');
  ...
  FirebaseAuthError.invalidCredential([String? message])
      : this('invalid-credential', message ?? 'Invalid credential object provided.');
  ...
  FirebaseAuthError.invalidDisabledField([String? message])
      : this('invalid-disabled-field', message ?? 'The disabled field must be a boolean.');
  ...
  FirebaseAuthError.invalidDisplayName([String? message])
      : this('invalid-display-name', message ?? 'The displayName field must be a valid string.');
  ...
  FirebaseAuthError.invalidEmail([String? message])
      : this('invalid-email', message ?? 'The email address is improperly formatted.');
  ...
  FirebaseAuthError.invalidEmailVerified([String? message])
      : this('invalid-email-verified', message ?? 'The emailVerified field must be a boolean.');
  ...
  FirebaseAuthError.invalidHashSaltSeparator([String? message])
      : this('invalid-hash-salt-separator',
  ...
  FirebaseAuthError.invalidIdToken([String? message])
      : this('invalid-id-token', message ?? 'The provided ID token is not a valid Firebase ID token.');

  /// The last sign-in time must be a valid UTC date string.
  FirebaseAuthError.invalidLastSignInTime([String? message])
      : this('invalid-last-sign-in-time', message ?? 'The last sign-in time must be a valid UTC date string.');

  /// The provided next page token in listUsers() is invalid. It must be a valid
  /// non-empty string.
  FirebaseAuthError.invalidPageToken([String? message])
      : this('invalid-page-token', message ?? 'The page token must be a valid non-empty string.');
  ...
  FirebaseAuthError.invalidPassword([String? message])
      : this('invalid-password', message ?? 'The password must be a string with at least 6 characters.');

  /// The password hash must be a valid byte buffer.
  FirebaseAuthError.invalidPasswordHash([String? message])
      : this('invalid-password-hash', message ?? 'The password hash must be a valid byte buffer.');

  /// The password salt must be a valid byte buffer
  FirebaseAuthError.invalidPasswordSalt([String? message])
      : this('invalid-password-salt', message ?? 'The password salt must be a valid byte buffer.');
  ...
  FirebaseAuthError.invalidPhotoUrl([String? message])
      : this('invalid-photo-url', message ?? 'The photoURL field must be a valid URL.');

  /// The providerData must be a valid array of UserInfo objects.
  FirebaseAuthError.invalidProviderData([String? message])
      : this('invalid-provider-data', message ?? 'The providerData must be a valid array of UserInfo objects.');

  /// The providerId must be a valid supported provider identifier string.
  FirebaseAuthError.invalidProviderId([String? message])
      : this('invalid-provider-id', message ?? 'The providerId must be a valid supported provider identifier string.');
  ...
  FirebaseAuthError.missingContinueUri([String? message])
      : this('missing-continue-uri', message ?? 'A valid continue URL must be provided in the request.');
  ...
  FirebaseAuthError.missingIosBundleId([String? message])
      : this('missing-ios-bundle-id', message ?? 'The request is missing an iOS Bundle ID.');

  /// A uid identifier is required for the current operation.
  FirebaseAuthError.missingUid([String? message])
      : this('missing-uid', message ?? 'A uid identifier is required for the current operation.');
  ...
  FirebaseAuthError.phoneNumberAlreadyExists([String? message])
      : this('phone-number-already-exists', message ?? 'The user with the provided phone number already exists.');
  ...
  FirebaseAuthError.projectNotFound([String? message])
      : this('project-not-found', message ?? 'No Firebase project was found for the provided credential.');
  ...
  FirebaseAuthError.forbiddenClaim([String? message])
      : this('reserved-claim', message ?? 'The specified developer claim is reserved and cannot be specified.');

  /// The provided Firebase session cookie is expired.
  FirebaseAuthError.sessionCookieExpired([String? message])
      : this('session-cookie-expired', message ?? 'The Firebase session cookie is expired.');

  /// The Firebase session cookie has been revoked.
  FirebaseAuthError.sessionCookieRevoked([String? message])
      : this('session-cookie-revoked', message ?? 'The Firebase session cookie has been revoked.');

  /// The provided uid is already in use by an existing user. Each user must
  /// have a unique uid.
  FirebaseAuthError.uidAlreadyExists([String? message])
      : this('uid-already-exists', message ?? 'The user with the provided uid already exists.');
  ...
  FirebaseAuthError.userNotFound([String? message])
      : this('user-not-found', message ?? 'There is no user record corresponding to the provided identifier.');

  //

  FirebaseAuthError.QUOTA_EXCEEDED([String? message])
      : this('quota-exceeded', message ?? 'The project quota for the specified operation has been exceeded.');
  FirebaseAuthError.TENANT_NOT_FOUND([String? message])
      : this('tenant-not-found', message ?? 'There is no tenant corresponding to the provided identifier.');
  FirebaseAuthError.UNSUPPORTED_TENANT_OPERATION([String? message])
      : this('unsupported-tenant-operation', message ?? 'This operation is not supported in a multi-tenant context.');
  FirebaseAuthError.MISSING_DISPLAY_NAME([String? message])
      : this(
            'missing-display-name', message ?? 'The resource being created or edited is missing a valid display name.');
  FirebaseAuthError.MISSING_ISSUER([String? message])
      : this('missing-issuer', message ?? 'The OAuth/OIDC configuration issuer must not be empty.');
  FirebaseAuthError.MISSING_OAUTH_CLIENT_ID([String? message])
      : this('missing-oauth-client-id', message ?? 'The OAuth/OIDC configuration client ID must not be empty.');
  FirebaseAuthError.MISSING_PROVIDER_ID([String? message])
      : this('missing-provider-id', message ?? 'A valid provider ID must be provided in the request.');
  FirebaseAuthError.MISSING_SAML_RELYING_PARTY_CONFIG([String? message])
      : this('missing-saml-relying-party-config',
  ...
  FirebaseAuthError.MISSING_CONFIG([String? message])
      : this('missing-config', message ?? 'The provided configuration is missing required attributes.');
  FirebaseAuthError.INVALID_TOKENS_VALID_AFTER_TIME([String? message])
      : this('invalid-tokens-valid-after-time',
            message ?? 'The tokensValidAfterTime must be a valid UTC number in seconds.');

  FirebaseAuthError.INVALID_TENANT_ID([String? message])
      : this('invalid-tenant-id', message ?? 'The tenant ID must be a valid non-empty string.');
  FirebaseAuthError.INVALID_TENANT_TYPE([String? message])
      : this('invalid-tenant-type', message ?? 'Tenant type must be either "full_service" or "lightweight".');

  FirebaseAuthError.INVALID_PROJECT_ID([String? message])
      : this('invalid-project-id',
  ...
  FirebaseAuthError.INVALID_NAME([String? message])
      : this('invalid-name', message ?? 'The resource name provided is invalid.');
  FirebaseAuthError.INVALID_OAUTH_CLIENT_ID([String? message])
      : this('invalid-oauth-client-id', message ?? 'The provided OAuth client ID is invalid.');
  //

  FirebaseAuthError.billingNotEnabled([String? message])
      : this('billing-not-enabled', message ?? 'Feature requires billing to be enabled.');

  FirebaseAuthError.CONFIGURATION_EXISTS([String? message])
      : this('configuration-exists', message ?? 'A configuration already exists with the provided identifier.');
  FirebaseAuthError.CONFIGURATION_NOT_FOUND([String? message])
      : this('configuration-not-found',
  ...
  FirebaseAuthError.INVALID_CONFIG([String? message])
      : this('invalid-config', message ?? 'The provided configuration is invalid.');
  FirebaseAuthError.MISMATCHING_TENANT_ID([String? message])
      : this('mismatching-tenant-id',
...
class FirebaseStorageError extends _PrefixedFirebaseError {
  FirebaseStorageError(String code, String? message) : super('storage', code, message);

  FirebaseStorageError.invalidArgument([String? message]) : this('invalid-argument', message);
}
